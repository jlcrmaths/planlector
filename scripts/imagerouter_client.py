...
def _post_runware_http(
    ...
    task = {
        ...
        "width": int(max(64, min(width, 1536))),
        "model": mdl,
    ...
    if resp.status_code == 401:
        raise ImageRouterError("Runware: 401 (API key inválida o ausente).")
    if resp.status_code >= 400:
        raise ImageRouterError(f"Runware HTTP {resp.status_code}: {resp.text[:300]}")
...
def _post_hf_bytes(endpoint: str, api_key: str, prompt: str, timeout: int) -> bytes:
    ...
    if resp.status_code in (503, 524):
        ...
        resp = requests.post(endpoint, headers=headers, json=payload, timeout=timeout)
    if resp.status_code >= 400:
        raise ImageRouterError(f"HF HTTP {resp.status_code}: {resp.text[:300]}")
...
def _post_openai_images(endpoint: str, api_key: str, payload: dict, timeout: int) -> bytes:
    ...
    if resp.status_code == 403:
        try:
            msg = (resp.json().get("error", {}).get("message") or "")[:500]
        except Exception:
            msg = resp.text[:500]
        if "deposit" in msg.lower():
            raise ImageRouterBillingRequired(msg)
    if resp.status_code >= 400:
        raise ImageRouterError(f"OpenAI-style HTTP {resp.status_code}: {resp.text[:300]}")
    data = resp.json()
...
def _post_aihorde_http(base_url: str, prompt: str, api_key: str, width: int, height: int, steps: int, timeout: int) -> bytes:
    ...
    if not req_id:
        raise ImageRouterError("AI Horde sin id de petición.")
    t0 = time.time()
    while True:
        ...
        if rc.json().get("done") == 1:
            break
        if time.time() - t0 > timeout:
            raise ImageRouterError("AI Horde: timeout.")
        time.sleep(2)
    ...
    if img_field.lower().startswith("http"):
        ...
        return _ensure_png(rimg.content)
    if img_field.lower().startswith("data:image/"):
...
def generate_image_via_imagerouter(
    ...
    if provider == "runware":
        ...
        if not api_key:
            raise ImageRouterError("Falta RUNWARE_API_KEY.")
